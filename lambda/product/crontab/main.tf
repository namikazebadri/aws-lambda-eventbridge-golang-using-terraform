...
resource "aws_lambda_function" "product_crontab" {
...
resource "aws_cloudwatch_event_target" "crontab_trigger_target" {
  rule      = aws_cloudwatch_event_rule.crontab_trigger.name
  target_id = "LambdaTarget"
  arn       = aws_lambda_function.product_crontab.arn
  input = jsonencode({
...
resource "aws_lambda_permission" "allow_event_bridge" {
  ...
  function_name = aws_lambda_function.product_crontab.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.crontab_trigger.arn
}

resource "aws_cloudwatch_log_group" "log_group" {
  name              = "/aws/lambda/${aws_lambda_function.product_crontab.function_name}"
  retention_in_days = 7
}